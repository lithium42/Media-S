    	...
    	if ($type_selector) {
			...
			OR      $woo_metabox['type'] == 'images') {
        	    ...
        	    elseif($woo_metabox['type'] == 'text'){
        	        ...
        	        $output .= "\t".'<tr>';
        	        $output .= "\t\t".'<th class="woo_metabox_names"><label for="'.$woo_id.'">'.$woo_metabox['label'].'</label></th>'."\n";
        	        $output .= "\t\t".'<td><input class="woo_input_text '.$add_class.'" type="'.$woo_metabox['type'].'" value="'.$woo_metaboxvalue.'" name="'.$woo_name.'" id="'.$woo_id.'"/>';
        	    ...
        	    elseif ($woo_metabox['type'] == 'textarea'){
        	        ...
        	        $output .= "\t\t".'<td><textarea class="woo_input_textarea '.$add_class.'" name="'.$woo_name.'" id="'.$woo_id.'">' . stripslashes($woo_metaboxvalue) . '</textarea>';
        	        $output .= '<span class="woo_metabox_desc">'.$woo_metabox['desc'] .' '. $add_counter.'</span></td>'."\n";
        	        $output .= "\t".'</tr>'."\n";  
    	...
    	if ( function_exists( 'get_post_types') ) {
			...
			foreach ($custom_post_list as $type){
				...
				//if(!empty($woo_metaboxes)) Temporarily Removed
					add_meta_box( 'woothemes-settings', get_option( 'woo_themename').' Custom Settings','woothemes_metabox_create',$type,'normal' );
				
				if(array_search($type, $seo_post_types) !== false){	
					if(get_option( 'seo_woo_hide_fields') != 'true'){
						add_meta_box( 'woothemes-seo',get_option( 'woo_themename').' SEO Settings','woothemes_metabox_create',$type,'normal','high','seo' );
					}
				}
			}
    	} else {
        	...
        	if(get_option( 'seo_woo_hide_fields') != 'true'){
        		add_meta_box( 'woothemes-seo',get_option( 'woo_themename').' SEO Settings','woothemes_metabox_create','post','normal','high','seo' );
        		add_meta_box( 'woothemes-seo',get_option( 'woo_themename').' SEO Settings','woothemes_metabox_create','page','normal','high','seo' );
    		}
    	}
...
.woo_metabox_fields .screenshot .mlu_remove{background:url( "<?php bloginfo( 'template_directory' ); ?>/functions/images/ico-delete.png") no-repeat scroll 0 0 transparent;border:medium none;bottom:-4px;display:block;float:left;height:16px;position:absolute;left:-4px;text-indent:-9999px;width:16px;padding:0;}
