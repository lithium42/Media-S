...
function woo_shortcode_box($atts, $content = null) {
   extract(shortcode_atts(array(	'type' => 'normal',
   	...
   	return '<div class="woo-sc-box '.$type.' '.$size.' '.$style.' '.$border.'"'.$custom.'>' . woo_remove_wpautop($content) . '</div>';
}
...
function woo_shortcode_contactform ( $atts, $content = null ) {
		...
		if ( $email == '' ) {} else {
			...
			if ( count( $formatted_dynamic_atts ) ) {
				...
				foreach ( $formatted_dynamic_atts as $k => $v ) {
					...
					if ( substr( $k, 0, 5 ) == 'text_' ) {
						...
						$html .= '<input type="text" value="' . ${$k} . '" name="' . $k . '" id="' . $k . '" class="txt input-text textfield woo-input-text" /></p>' . "\n";
			...
			$html .= '<p class="screenReader"><label for="checking" class="screenReader">' . __( 'If you want to submit this form, do not enter anything in this field', 'woothemes' ) . '</label><input type="text" name="checking" id="checking" class="screenReader" value="' . $checking . '" /></p>' . "\n";
			
			$html .= '<p class="buttons"><input type="hidden" name="submitted" id="submitted" value="true" /><input class="submit button" type="submit" value="' . __( 'Submit', 'woothemes' ) . '" /></p>';
